...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--cuda", default=True, type=str2bool, help="Use cuda to evaulate model"
    )
    parser.add_argument(
        "--fast_nms",
        default=True,
    ...
    parser.add_argument(
        "--output_coco_json",
        dest="output_coco_json",
        action="store_true",
        help="If display is not set, instead of processing IoU values, this just dumps detections into the coco json file.",
    )
    parser.add_argument(
        "--bbox_det_file",
        default="results/bbox_detections.json",
        type=str,
        help="The output file for coco bbox results if --coco_results is set.",
    )
    parser.add_argument(
        "--mask_det_file",
        default="results/mask_detections.json",
        type=str,
    ...
    parser.add_argument(
        "--image", default=None, type=str, help="A path to an image to use for display."
    )
    parser.add_argument(
        "--images",
        default=None,
...
def prep_benchmark(dets_out, h, w):
    with timer.env("Postprocess"):
        t = postprocess(
            dets_out, w, h, crop_masks=args.crop, score_threshold=args.score_threshold
        )
...
class Detections:
    ...
    def dump_web(self):
        ...
        config_outs = [
            ...
            "use_prediction_module",
            "use_yolo_regressors",
            "use_prediction_matching",
            "train_masks",
        ]

        output = {"info": {"Config": {key: getattr(cfg, key) for key in config_outs},}}
            ...
            image_obj["dets"].append(
                {
                    ...
                    "category": cfg.dataset.class_names[
                        get_transformed_cat(bbox["category_id"])
                    ],
...
def evalvideo(net: Yolact, path: str, out_path: str = None):
            ...
            while running:
                ...
                if not vid_done:
                    ...
                    new_target = (
                        ...
                        if is_webcam
                        else max(frame_time_stabilizer, frame_time_target)
                    )
                else:
    ...
    extract_frame = lambda x, i: (
        x[0][i]
        if x[1][i]["detection"] is None
        else x[0][i].to(x[1][i]["detection"]["box"].device),
        [x[1][i]],
    )
    ...
    active_frames = [
        {"value": extract_frame(first_batch, i), "idx": 0}
    ...
    try:
        while vid.isOpened() and running:
            ...
            fps_str = (
                "Processing FPS: %.2f | Video Playback FPS: %.2f | Frames in Buffer: %d"
